...
|-----------|------|--------|
| `scout-rest-api` skill | `plugins/atlasrepo/skills/scout-rest-api/SKILL.md` | ✅ ready |
...
The stdio MCP server (`search_workflow_stories`, `recommend_tool_stack`, `find_self_hosted_alternatives`, `get_implementation_guide`, `get_story`, `list_tools`) currently lives in the main AtlasRepo repository and requires a local build. Once it ships to npm as `atlasrepo-mcp`, rename `plugins/atlasrepo/mcp.json.example` to `.mcp.json` and the plugin will register it automatically. Manual registration will then be:
